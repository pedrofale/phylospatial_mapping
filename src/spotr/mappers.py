...
def fgwot(single_cell_data, spatial_data, tree_distance_matrix, space_distance_matrix, alpha=0.5, epsilon=1e-1):
    ...
    T_fgw = entropic_fused_gromov_wasserstein(M, C1, C2, w1, w2, loss_fun='square_loss', alpha=alpha, epsilon=epsilon)
    return T_fgw
...
def gwot(tree_distance_matrix, space_distance_matrix):
    ...
    w1 = ot.unif(C1.shape[0])
    w2 = ot.unif(C2.shape[0])
    T_gw = entropic_gromov_wasserstein(C1, C2, w1, w2, loss_fun='square_loss', epsilon=1e-1)
    return T_gw
